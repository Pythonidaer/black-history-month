...
/** Section 7: text left, image right; Artistic Identity + Later Life; Vibrant background. */
export default function SectionSeven() {
  const section7 = figure.section7
  const [gradient, setGradient] = useState(FALLBACK_GRADIENT)
  const [gradientReady, setGradientReady] = useState(false)
  const imgRef = useRef(null)

  const handleImageLoad = useCallback(() => {
    const source = imgRef.current ?? section7?.image
    if (!source) return
    Vibrant.from(source)
      .getPalette()
      .then((palette) => {
  ...
  if (!section7?.blocks?.length) return null

  const imageSrc = section7.image || '/nina_facing_blue_full.JPG'

  return (
    <section
      id="section-7"
      className="section section--seven-bio"
      aria-label="Section 7"
      style={{ background: FALLBACK_GRADIENT }}
    >
      <div
        className="section--seven-bio__gradient-overlay"
        style={{
          background: gradient,
        ...
        aria-hidden
      />
      <div className="section--seven-bio__right">
        {section7.blocks.map((block, i) => (
          <Fragment key={i}>
            {i > 0 && <div className="cinematic-gold-divide" aria-hidden="true" />}
            <div className="section--seven-bio__block">
              <h2 className="section--seven-bio__block-title">{block.title}</h2>
              <div className="section--seven-bio__block-body">
      ...
      <div className="section--seven-bio__media">
        <div className="section--seven-bio__img-wrap">
          <img
            ref={imgRef}
            src={imageSrc}
            alt="Nina Simone"
            className="section--seven-bio__img"
